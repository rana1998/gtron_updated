...
{
    $response = AdminHelper::getWeeklyUserWithdrawalHistory($db);
} else if(isset($filter) && $filter =='monthly') {
    $response = AdminHelper::getMonthlyUserWithdrawalHistory($db);
}else if(isset($filter) && $filter =='day-internal') {
    $start_date = $_GET['start_date'];
    $end_date = $_GET['end_date'];
    $response = AdminHelper::getIntervalUserWithdrawalHistory($db, $start_date, $end_date);
} else {
