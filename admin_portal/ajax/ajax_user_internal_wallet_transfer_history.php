...
{
    $response = AdminHelper::getWeeklyInternalWalletTransferLogs($db);
} else if(isset($filter) && $filter =='monthly') {
    $response = AdminHelper::getMonthlyInternalWalletTransferLogs($db);
} else if(isset($filter) && $filter =='day-internal') {
    $start_date = $_GET['start_date'];
    $end_date = $_GET['end_date'];
    $response = AdminHelper::getIntervalInternalWalletTransferLogs($db, $start_date, $end_date);
}
else {
