...
{
    $response = AdminHelper::getWeeklyUserXAmountWalletHistory($db);
} else if(isset($filter) && $filter =='monthly') {
    $response = AdminHelper::getMonthlyUserXAmountWalletHistory($db);
} else if(isset($filter) && $filter =='day-internal') {
    $start_date = $_GET['start_date'];
    $end_date = $_GET['end_date'];
    $response = AdminHelper::getIntervalUserXAmountWalletHistory($db, $start_date, $end_date);
} else {
