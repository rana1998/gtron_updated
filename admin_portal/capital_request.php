...
<!--                                    <div class="card-header table-card-header">
      ...
      <table id="basic-btn" class="table table-striped table-bordered nowrap">
          ...
          <tbody>
          <?php 
              $select= "SELECT * FROM `capital_request` WHERE status = 'Pending'";
              ...
              while($data=mysqli_fetch_array($res)) : ?>
                    <tr>
                      ...
                      <td> 
                             <a href="capital_request.php?approved_btn=<?php echo $data['id']; ?>&action=Approved"  class="btn btn-success btn-sm">Approve</a>
    ...
    <?php 
          if (isset($_GET['approved_btn']) && $_GET['action'] == 'Approved') {
              ...
              $q="select * from `capital_request` where id = '$id'";
              $result=mysqli_query($con,$q);
              $data = mysqli_fetch_assoc($result);
              
              $fetchDays = $data['days'];
              $fetchPkgPrice = $data['pkg_price'];
              $fetchUserName = $data['user_name'];
              
              echo "<script>alert(".$fetchPkgPrice.")</script>";
              echo "<script>alert(".$fetchDays.")</script>";
            //   die();
              if($fetchDays < 180)
              {
                  $pkgPrice = $fetchPkgPrice * 0.75;
                  
                //   echo "<script>alert(".$pkgPrice.")</script>";
                
                //update user_registration
                $qy="update user_registration set current_balance = current_balance + '$pkgPrice' where user_name = '$fetchUserName'";
                $result = mysqli_query ($con,$qy);
                
                //insert wallet summary
                $insertWalletQuery= "INSERT INTO `wallet_summary`(`user_name`, `amount`, `description`, `wallet_type`, `type`) 
                                    VALUES ('$fetchUserName','$pkgPrice','Capital Refund','Cash Wallet','Credit')";
                $walletResult= mysqli_query($con,$insertWalletQuery);
                
                //update capital request
                
                $updateCapitalRequest = "update `capital_request` set `status`='Delivered' where id='$id'";
                $resultUpdateCapital= mysqli_query($con,$updateCapitalRequest);
                
                
                $_SESSION['successMsg'] = 'Capital Request Updated Successfully';
                header('location:capital_request.php');
                exit();
                
              }
               elseif($fetchDays > 180 && $fetchDays <= 365)
              {
                  $pkgPrice = $fetchPkgPrice * 0.85;
                //   echo "<script>alert(".$pkgPrice.")</script>";
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>Withdrawal Rejected | Infinite Trade Group</title>
        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse; width: 100%; max-width: 600px;" class="content">
            ...
            <tr>
                <td align="left" bgcolor="#ffffff" style="padding: 30px 10px 0px 10px; color: #ffffff; font-family: Arial, sans-serif; font-size: 20px; font-weight: bold;">
                    <img src="https://infinitetradegroup.com/data/assets/images/logo-icon.png" alt="Withdrawal Rejected" width="114" height="80" style="display:block; margin-bottom: 15px;">
    ...
    if( send_email($param) ){
          $_SESSION['errorMsg']='Withdrawal Rejected';
                  header("Location: pending_withdrawal.php");
                  exit();
